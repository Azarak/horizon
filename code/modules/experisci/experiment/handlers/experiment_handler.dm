...
/datum/component/experiment_handler/proc/get_available_servers(turf/turf_to_check_for_servers = null)
	...
	if (!SSmapping.virtual_level_trait(turf_to_check_for_servers,ZTRAIT_STATION))
	...
	for (var/obj/machinery/rnd/server/server in SSresearch.servers)
		...
		if (position_of_this_server_machine && SSmapping.virtual_level_trait(position_of_this_server_machine,ZTRAIT_STATION))
